...
import { assertValidString } from '../utils.js';
import { tryGetValue } from './index.js';


export function getArray(source: Source, offset: number, convertor: Convertor): ValueResult | null {
  ...
  while (true) {
    const valueResult = tryGetValue(source, nextIndex, convertor);
    if (valueResult === null) {
      ...
      if (m[4]) {
        assertValidString(m[3]!, source, m[1]!.length + 1);
        source.next();
        nextIndex = 0;
        continue;
      }
...
function checkEndOrContinue(source: Source, offset: number): EndOrContinueState | null {
  ...
  if (matches[2]) {
    ...
    if (matches[4]) {
      if (matches[3]) {
        assertValidString(matches[3], source, source.line.indexOf('#', offset) + 1);
      }
      source.next();
    } else {
      nextIndex = RE_ARRAY_END_OR_CONTINUE.lastIndex;
