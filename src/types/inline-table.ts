...
export function getInlineTable(source: Source, offset: number, convertor: Convertor): ValueResult | null {
  ...
  while (true) {
    const key = getKey(source, nextIndex, RE_KEY);
    if (key) {
      const valueResult = tryGetValue(source, key.nextIndex, convertor);
      if (valueResult === null) {
